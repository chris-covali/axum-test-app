...
impl BinanceClient {
    ...
    fn map_val_to_kline(val: serde_json::Value, symbol: String, interval: String) -> Kline {
        Kline {
            ...
            open: val[1].as_str().unwrap().parse::<f32>().unwrap(),
            high: val[2].as_str().unwrap().parse::<f32>().unwrap(),
            low: val[3].as_str().unwrap().parse::<f32>().unwrap(),
            close: val[4].as_str().unwrap().parse::<f32>().unwrap(),
            volume: val[5].as_str().unwrap().parse::<f32>().unwrap(),
            close_time: DateTime::from_timestamp_millis(val[6].as_i64().unwrap()).unwrap(),
            quote_asset_volume: val[7].as_str().unwrap().parse::<f32>().unwrap(),
            number_of_trades: val[8].as_i64().unwrap(),
            taker_buy_base_asset_volume: val[9].as_str().unwrap().parse::<f32>().unwrap(),
            taker_buy_quote_asset_volume: val[10].as_str().unwrap().parse::<f32>().unwrap(),
            indicators: None,
        }
    }
}
